...
- `__pycache__/`
- `*.pyc`
- temporary Word files such as `~$*.docx`
- generated vector indexes or local cache folders
...
Git tracks files, not empty folders. This means an empty folder will not appear on GitHub unless it contains at least one file.

To create a folder for GitHub:

1. Create the folder locally.
2. Add at least one file inside it.
3. Commit and push the folder with that file.
...
If you need Git to keep an otherwise empty folder, add a placeholder file:
...
Common GitHub folder structure for this project:
...
Once these folders contain files and are committed, GitHub will show them automatically.

## Disclaimer

The sample policy documents in this project are fictional and intended for demonstration and evaluation purposes only.
